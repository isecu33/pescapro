/* PescaPro - Capa de datos: Open-Meteo (gratuito, sin API key) + caché offline.
   ...
   Los datos se refrescan periódicamente (CONFIG.REFRESH_MS) y se cachean
...
/* fetch con timeout (fix HIGH de auditoria: antes no tenia limite y podia
   colgar el refresco indefinidamente con conexion intermitente). */
async function getJSON(url) {
  ...
  try {
    ...
    if (e.name === 'AbortError') {
      throw new Error('Tiempo de espera agotado al conectar con ' + url.split('?')[0]);
    }
    throw e;
  } finally {
    clearTimeout(timer);
...
export function fetchClima(lat, lon) {
  const url = CONFIG.API.clima + '?' + qs({
    latitude: lat.toFixed(4),
    longitude: lon.toFixed(4),
    hourly: CONFIG.HOURLY_CLIMA.join(','),
    daily: CONFIG.DAILY_CLIMA.join(','),
    timezone: 'auto',
    past_days: CONFIG.DIAS_PASADOS,
    forecast_days: CONFIG.DIAS_PREVISION
  });
  return getJSON(url);
}
...
export function fetchMarino(lat, lon) {
  const url = CONFIG.API.marino + '?' + qs({
    latitude: lat.toFixed(4),
    longitude: lon.toFixed(4),
    hourly: CONFIG.HOURLY_MARINO.join(','),
    timezone: 'auto',
    past_days: CONFIG.DIAS_PASADOS,
    forecast_days: CONFIG.DIAS_PREVISION,
    cell_selection: 'sea'
...
export function fetchCorrientesGrid(lat, lon) {
  const n = CONFIG.GRID_N, paso = CONFIG.GRID_STEP, medio = (n - 1) / 2;
...
export async function cargarTodo(lat, lon) {
...
export function desdeCache(lat, lon) {
  try {
