...
PP.mareas = (function () {
  ...
  function amplitud(exts, ahora) {
    ...
    const clase = PP.MAREA_CLASES.find(c => rango <= c.max) || PP.MAREA_CLASES[PP.MAREA_CLASES.length - 1];
    // Coeficiente orientativo estilo 20-120 mapeando el rango típico del Cantábrico (1.0–4.6 m)
    const coef = Math.round(U().clamp(20 + (rango - 1.0) / (4.6 - 1.0) * 100, 20, 120));
  ...
  function estadoEn(fecha, horas, exts) {
    ...
    const flujo = Math.sin(Math.PI * PP.util.clamp(frac, 0, 1));
  ...
  return { extremos, amplitud, estadoEn, analizar };
})();
